...
# def getSeqGraph(seq):
    ...
    return Data(x=x, edge_index=edge_index, num_nodes=len(nodes), mean_interv=mean_interv, edge_dist=dist_interv)
...
class GraphData(Dataset):
    ...
    def __getitem__(self, index):
        if not self.is_eval:
            ...
            uid, poi, seq, seq_time, cur_time = self.seq_data[index]
...
def getDatasets(path='../data/processed', dataset='nyc'):
